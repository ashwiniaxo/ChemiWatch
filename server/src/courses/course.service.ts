...
export class CourseService {
  ...
  async getCourseAvailability(
    ...
    const accessResponse =
      await this.cheminotClient.validateCourse(normalizedCode);
    ...
    const offeredCourses =
      await this.cheminotClient.getOfferedGroups(normalizedCode);

    const course = offeredCourses.CoursOfferts.find(
      (item) => item.Sigle === normalizedCode,
    );
    ...
    if (!course) {
      return {
        ...
        groups: [],
        message:
          offeredCourses.Message ??
          "No available groups were found for this course.",
      };
    }
    ...
    return {
      ...
      groups,
      ...(offeredCourses.Message
        ? { message: offeredCourses.Message }
        : {}),
    };
  }
}